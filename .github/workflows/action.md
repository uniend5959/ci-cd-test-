...
jobs:
  ...
  echo:
    ...
    steps:
      ...
      - name: 메인 브런치에만 실행되는 step
        if: github.ref_name == 'main'
        run: echo ' 메인 브런치 ${{ github.ref_name}} ' #로그남기기
  ...
  event:
    ...
    steps:
      - name: push 이벤트
        ...
        run: echo `메인 브런치 ${{ toJson(github.event)}}

      - name: pull rq 이벤트
        if: github.event_name == ' pull_request'
        run: echo `풀 리퀘 브런치 ${{ toJson(github.event)}}
